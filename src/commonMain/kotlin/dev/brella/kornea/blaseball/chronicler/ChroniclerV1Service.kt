...
interface ChroniclerV1Service : ChroniclerService {
    ...
    ): List<ChroniclerGameWrapper> {
        //0 also counts as infinite from the looks of things, which is **not** what we want
        if (count < 0 && count != YES_BRELLA_I_WOULD_LIKE_UNLIMITED_GAMES) return emptyList()

        return unwrapData(client.get("$chroniclerV1BaseUrl/games") {
    ...
    suspend fun getGameUpdates(
        ...
        started: Boolean? = null,
        season: Int? = null
    ): ChroniclerUpdateResponse? {
        //0 also counts as infinite from the looks of things, which is **not** what we want
        if (count < 0 && count != YES_BRELLA_I_WOULD_LIKE_UNLIMITED_GAMES) return null

        return client.get("$chroniclerV1BaseUrl/games/updates") {
            if (count != YES_BRELLA_I_WOULD_LIKE_UNLIMITED_GAMES) {
